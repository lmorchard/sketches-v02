...
export class CollisionEntity extends AreaEntity {
  ...
  findCircleVsCircleCollisionWith(otherEntity) {
    const {
      Position: position,
      AreaCircle: { radius },
    } = this;
  ...
  return (world) => {
    ...
    for (const eid of collisionQuery(world)) {
      ...
      for (const otherEid of otherEids) {
        ...
        const collisionMetadata = entity.findCollisionWith(otherEntity);
        if (collisionMetadata) {
          collisionService.add(eid, otherEid, collisionMetadata);
        }
      }
    }
    return world;
...
export const collisionDebugRenderer = (options = {}) => {
  ...
  return (world) => {
    ...
    for (const [eid, otherEid, collisionData] of collisionService.allPairs()) {
      ...
      g.drawCircle(edge.x, edge.y, 5);
      g.drawCircle(otherEdge.x, otherEdge.y, 5);
      g.drawCircle(collisionCenter.x, collisionCenter.y, 5);
      g.drawCircle(otherCollisionCenter.x, otherCollisionCenter.y, 5);
