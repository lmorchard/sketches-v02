...
export const WORLD_POSITION_INDEX = Symbol("positionIndex");
...
export class PositionIndexService {
  ...
  get quadtree() {
    return (
      this.world &&
      this.world[WORLD_POSITION_INDEX] &&
      this.world[WORLD_POSITION_INDEX].quadtree
    );
  }
...
export const positionIndexSystem = (options = {}) => {
  ...
  return (world) => {
    if (!world[WORLD_POSITION_INDEX]) {
      world[WORLD_POSITION_INDEX] = {};
    }
    ...
    world[WORLD_POSITION_INDEX].quadtree = quadtree;
