...
export const bounceSystem = (options = {}) => {
  ...
  const main = (world) => {
    ...
    for (const [eid, otherEid] of collisionService.allPairs()) {
      ...
      if (!collisionService.stillHasPair(eid, otherEid)) {
        ...
        resolveCollision(options);
      }

      // Ensure "sticky" objects are forced apart.
      applySeparationForce(options);
    }
  ...
  function applySeparationForce({ separationFactor = 0.8 } = {}) {
    ...
    if (separationDistance < separationGoal) {
      ...
      const totalForce =
        separationFactor * (separationGoal - separationDistance);

      separationVector.scaleBy(totalForce * (bounce.mass / totalMass));
      velocity.add(separationVector);

      otherSeparationVector.scaleBy(
        totalForce * (otherBounce.mass / totalMass)
      );
      otherVelocity.add(separationVector.scaleBy(-1));
    }
  }
