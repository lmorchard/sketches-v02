...
export function init() {
  ...
  Object.assign(world, {
    ...
    addToPane(pane) {
      pane.addButton({ title: "Sketches Home" }).on("click", () => {
        const home = document.head.querySelector("link[rel=home]");
        if (home) {
          location.href = home.getAttribute("href");
        } else {
          location.href = "../../../../index.html";
        }
      });
      pane.addButton({ title: "Screenshot" }).on("click", () => {
        world.captureSnapshotPNG();
      });
...
export class WorldService {
  static storeKey = Symbol("worldServiceKey");
  static attributes = [];
