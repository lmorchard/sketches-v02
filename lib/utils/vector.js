...
export class Line2D {
  ...
  static fromAtoB = new Vector2D();
  static fromAtoP = new Vector2D();

  findClosestPoint(otherPoint, rv = new Point2D()) {
    const { fromAtoB, fromAtoP } = this.constructor;
    const { x: otherX, y: otherY } = otherPoint;
    const { x: startX, y: startY } = this.p0;
    const { x: endX, y: endY } = this.p1;

    // https://jsfiddle.net/soulwire/UA6H5/
    fromAtoB.set(endX - startX, endY - startY);
    fromAtoP.set(otherX - startX, otherY - startY);

    const len = fromAtoB.lengthSquared();
    const dot = fromAtoP.dot(fromAtoB);
    const t = Math.min(1, Math.max(0, dot / len));

    return rv.set(startX + fromAtoB.x * t, startY + fromAtoB.y * t);
  }

  static v0 = new Vector2D();

  findClosestPointsBetweenLines(otherLine, rv = new Line2D()) {
    const { v0 } = this.constructor;

    // https://wickedengine.net/2020/04/26/capsule-collision-detection/
    const d0 = v0.copy(otherLine.p0).subtract(this.p0).lengthSquared();
    const d1 = v0.copy(otherLine.p1).subtract(this.p0).lengthSquared();
    const d2 = v0.copy(otherLine.p0).subtract(this.p1).lengthSquared();
    const d3 = v0.copy(otherLine.p1).subtract(this.p1).lengthSquared();

    const bestA = d2 < d0 || d2 < d1 || d3 < d0 || d3 < d1 ? this.p1 : this.p0;

    otherLine.findClosestPoint(bestA, rv.p1);
    this.findClosestPoint(rv.p1, rv.p0);
